// Creates the "credential-change" companion Workflow: disables PRISM +
// Active Directory accounts, then creates a certification campaign for
// the identity's manager to review -- combining the two proven native
// remediation patterns from earlier today (PRISM-disable Workflows, and
// the reviewerCertificationType:"IDENTITY" campaign fix). No external
// HTTP calls, no embedded credentials -- native actions only.
...
async function main() {
  ...
  const body = {
    name: "SSF Injector Demo - Remove Access + Certify When Credential Changes",
    description:
      "Triggered by a CAEP Credential Change event with change_type=revoke. Disables PRISM + Active Directory accounts, then creates a certification campaign for the identity's manager.",
    trigger: {
      type: "EVENT",
      attributes: {
        id: "idn:caep-credential-change-events",
        "filter.$":
          '$.ssfEvent.events["https://schemas.openid.net/secevent/caep/event-type/credential-change"][?(@.change_type == "revoke")]',
      },
    },
    definition: {
      start: "Get Identity",
      steps: {
        ...
        "Send Email": {
          ...
          attributes: {
            body: "<p>Hello ${var1},</p><p>This email is to inform that a credential change (revocation) was detected for your direct report, ${var2}. PRISM and Active Directory accounts have been disabled, and a certification campaign named ${var4} has been created. Please review within 30 days.</p><p>Thank you,<br>Corporate Compliance Officer</p>",
            ...
            subject: "Credential Change Detected",
          },
